 ...
 * Integrates shadow effects and visual effects
 */
...
import ShadowEffects, { ShadowEffect } from './ShadowEffects';
...
const ShadowSystem: React.FC<ShadowSystemProps> = ({
  ...
  const [appliedEffects, setAppliedEffects] = useState<ShadowEffect[]>([]);
      ...
      <div className="absolute top-16 left-2 right-2 bottom-2 z-10">
        ...
        {activeTab === 'presets' && (
          <div className="h-full bg-white rounded-lg shadow-lg border border-gray-200 p-4">
            <h4 className="font-semibold mb-4">Shadow Presets</h4>
            <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
              {[
                { name: 'Soft Shadow', icon: Sun, color: 'bg-gray-100 text-gray-700' },
                { name: 'Hard Shadow', icon: Moon, color: 'bg-gray-100 text-gray-700' },
                { name: 'Glow Effect', icon: Zap, color: 'bg-yellow-100 text-yellow-700' },
                { name: 'Outline', icon: Circle, color: 'bg-blue-100 text-blue-700' },
                { name: 'Bevel', icon: Square, color: 'bg-purple-100 text-purple-700' },
                { name: 'Emboss', icon: Triangle, color: 'bg-pink-100 text-pink-700' },
                { name: 'Inner Shadow', icon: Heart, color: 'bg-red-100 text-red-700' },
                { name: 'Colored Shadow', icon: Star, color: 'bg-indigo-100 text-indigo-700' }
              ].map(preset => (
                <div
                  key={preset.name}
        ...
        {activeTab === 'settings' && (
          <div className="h-full bg-white rounded-lg shadow-lg border border-gray-200 p-4">
            <h4 className="font-semibold mb-4">Shadow System Settings</h4>
